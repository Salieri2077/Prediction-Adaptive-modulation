...
# 适应度函数
def fitness(chromosome, channel_states, snrs, A_s):
...
def genetic_algorithm(channel_states, snrs, buffer_state, arrival_rate, max_cross_time, A_s, population_size=50, K=25):
    ...
    while cross_time < max_cross_time:
        fitnesses = np.array([fitness(chrom, channel_states, snrs, A_s) for chrom in population], dtype=object)
    ...
    best_chromosome = max(population, key=lambda chrom: fitness(chrom, channel_states, snrs, A_s))
...
def genetic_algorithm_main(file_name, snrs,buffer_state, arrival_rate, max_cross_time, A_s):
    ...
    for i in range(0, len(channel_states), 24):
        ...
        if len(window_channel_states) == 24:
            best_modulation_scheme = genetic_algorithm(window_channel_states, window_snrs, buffer_state, arrival_rate, max_cross_time, A_s)
